    ...
    this.astInstructions.forEach((node, i) => {
      ...
      if (~index) {
        ...
        if (!(label in this.symbols)) {
          throw new Error(`Missing Symbol: "${label}"`)
        }

        const addr = this.symbols[label]
        let immediateValue: number = 0

        if (node.name === 'beq' || node.name === 'bne') {
          // TODO: Validiate?
          //  1. It's an address of an instruction
          //  2. It's not a partial

          // pc address
          const pcAddress = BASE_TEXT_ADDR + (i + 1) * 4
          const offset = addr - pcAddress
          immediateValue = offset / 4
        } else {
          if (addrNode.partitioned) {
            immediateValue = addrNode.part === 'upper'
              ? (addr & 0xffff0000) >>> 16
              : (addr & 0x0000ffff)
          } else {
            immediateValue = addr
